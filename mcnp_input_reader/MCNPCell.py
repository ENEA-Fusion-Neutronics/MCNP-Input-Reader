        ...
        with open(inputfile, errors='ignore') as f:
            ...
            for n, line in enumerate(f,1):
                ...
                if section_id==0:
                    ...
                    #if line.strip()=='':
                    #    continue
                    #if re.match('^C|c', line.strip()):
                    #    continue
                    #if re.match(r'\S', line):
                    #    section_surfs.append([[n],line])
                    #elif section_surfs:
                    #    section_surfs[-1][0].append(n)
                    #    section_surfs[-1][1]+=f' {line}'
                elif section_id==2:
                    pass
                    #if line.strip()=='':
                    #    continue
                    #if re.match('^C|c', line.strip()):
                    #    continue
                    #if re.match('^TR\d+|tr\d+|\*TR\d+|\*tr\d+', line):
                    #    start_tr=1
                    #    start_mat=0
                    #    section_tranfs.append([[n],line])
                    #elif section_tranfs and start_tr==1:
                    #    section_tranfs[-1][0].append(n)
                    #    section_tranfs[-1][1]+=f' {line}'
                    #if re.match('^M\d+|m\d+', line):
                    #    start_tr=0
                    #    start_mat=1
                    #    section_mats.append([[n],line])
                    #elif section_mats and start_mat==1:
                    #    section_mats[-1][0].append(n)
                    #    section_mats[-1][1]+=f' {line}'
        
                else:
                    break
        return cells

    funzione('/home/giovanni/lavoro/ITER/ITER_lower_port_14_sector7/E-lite_R200430')
